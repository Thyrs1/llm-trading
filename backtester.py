...
# Import all the REAL methods from your ai_processor and database_manager
...
class LiveAIStrategy(bt.Strategy):
    """
    This strategy makes a LIVE API call to the AI on each candle and uses the AI's
    risk parameter for position sizing.
    """
    def __init__(self):
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            ...
            pnl_pct = (trade.pnlcomm / self.trade_entry_price) if self.trade_entry_price else 0.0
            side = "LONG" if trade.history[0].event.size > 0 else "SHORT"
            
            log_trade(
                symbol=self.d5m._name, side=side, entry_price=self.trade_entry_price,
                exit_price=trade.price, quantity=trade.size, pnl=trade.pnlcomm,
    ...
    def next(self):
        ...
        self.log("Requesting AI decision...")
        ...
        if not self.position and action == 'OPEN_POSITION' and confidence == 'high':
            ...
            if not all([side, sl, tp]):
                self.log("AI decision to OPEN is missing side, sl, or tp.")
                return
            ...
            if side == 'LONG':
                ...
                self.order = self.buy_bracket(size=size, price=current_price, stopprice=sl, limitprice=tp)
            elif side == 'SHORT':
                self.log(f"AI DECISION: GO SHORT, Size: {size:.4f}, Risk: {risk_to_use*100:.2f}%")
                self.order = self.sell_bracket(size=size, price=current_price, stopprice=sl, limitprice=tp)
        
        elif self.position:
            # Bracket orders manage exits
            pass
        else:
            self.log(f"AI DECISION: {decision.get('action', 'WAIT')}")
